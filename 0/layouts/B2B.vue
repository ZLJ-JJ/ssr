 ...
 * @LastEditTime: 2021-09-03 13:46:36
...
<template>
  <div>
    <div class="layout_modal">
      <nuxt />
      <div style="height: 60px" v-if="!isApp"></div>
      <Footer v-if="!isApp" />
    </div>
  </div>
</template>
...
export default {
  ...
  created() {
    if (process.client) {
      var webUrl = navigator.userAgent;
      if (webUrl.indexOf("tospino;") != -1) {
        var userinfoShop = JSON.parse(webUrl.split("tospino;")[1]);
        var token = userinfoShop.token;
